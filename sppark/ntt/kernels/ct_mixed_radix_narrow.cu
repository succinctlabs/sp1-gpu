             ...
             bool is_intt, const fr_t d_domain_size_inverse)
{
    ...
    d_inout += column << lg_domain_size;  // already + lg_blowup
...
public:
    CT_launcher(fr_t* d_ptr, int lg_dsz, bool intt,
      ...
      : d_inout(d_ptr), lg_domain_size(lg_dsz), is_intt(intt), stage(0),
        ntt_parameters(params), poly_count(poly_count), stream(s)
    {}
    ...
    void step(int iterations)
    {
        ...
        size_t shared_sz = sizeof(fr_t) << (radix - 1); // size * 2^(radix-1)
        ...
        if (num_blocks < Z_COUNT)
            _CT_NTT<1><<<dim3(num_blocks, poly_count), block_size, shared_sz, stream>>>(NTT_ARGUMENTS);
        else if (stage == 0 || lg_domain_size < 12)
            _CT_NTT<Z_COUNT><<<dim3(num_blocks/Z_COUNT, poly_count), block_size, Z_COUNT*shared_sz, stream>>>(NTT_ARGUMENTS);
        else
            _CT_NTT<Z_COUNT, true><<<dim3(num_blocks/Z_COUNT, poly_count), block_size, Z_COUNT*shared_sz, stream>>>(NTT_ARGUMENTS);
