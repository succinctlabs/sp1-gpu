...
__host__ __device__ constexpr uint64_t HEIGHTTOBIT(int d) { return (uint64_t)1 << d; }
...
class FanPaarTowerFieldBase {
	static_assert(HEIGHT <= 6, "Height cannnot be larger than 6");

public:
	static constexpr uint64_t ONE() { return 1; }

	static constexpr uint64_t ZERO() { return 0; }

	static constexpr uint64_t N_BITS() { return HEIGHTTOBIT(HEIGHT); }

	static constexpr bool is_valid(uint64_t a) { return (a >> HEIGHTTOBIT(HEIGHT)) == 0; }

	static constexpr uint64_t add(uint64_t a, uint64_t b) { return a ^ b; }

protected:
	__host__ __device__ static inline uint64_t generic_multiply(uint64_t a, uint64_t b) {
		...
		uint64_t z0 = FanPaarTowerField<HEIGHT - 1>::multiply(a0, b0);
		uint64_t z2 = FanPaarTowerField<HEIGHT - 1>::multiply(a1, b1);
		uint64_t z1 = FanPaarTowerField<HEIGHT - 1>::multiply(a0 ^ a1, b0 ^ b1) ^ z0 ^ z2;
		uint64_t z2a = FanPaarTowerField<HEIGHT - 1>::multiply_alpha(z2);

		return from_subfield(z0 ^ z2, z1 ^ z2a);
	}

	__host__ __device__ static inline uint64_t generic_square(uint64_t a) {
		static_assert(HEIGHT > 0, "Height cannot be negative");
		auto aa = to_subfield(a);
		auto a0 = aa.first;
		auto a1 = aa.second;
		uint64_t z0 = FanPaarTowerField<HEIGHT - 1>::square(a0);
		uint64_t z2 = FanPaarTowerField<HEIGHT - 1>::square(a1);
		uint64_t z2a = FanPaarTowerField<HEIGHT - 1>::multiply_alpha(z2);
		return from_subfield(z0 ^ z2, z2a);
	}

	__host__ __device__ static inline uint64_t generic_inverse(uint64_t a) {
		static_assert(HEIGHT > 0, "Height cannot be negative");
		if (a == 0) {
			assert(a);
			return 0;
	...
	__host__ __device__ static inline uint64_t generic_multiply_alpha(uint64_t a) {
		static_assert(HEIGHT > 0, "Height cannot be negative");
		// uint64_t mask = BITMASK(HEIGHTTOBIT(height));
		// a &= mask;

		auto aa = to_subfield(a);
		auto a0 = aa.first;
		auto a1 = aa.second;
		uint64_t z1 = FanPaarTowerField<HEIGHT - 1>::multiply_alpha(a1);
		return from_subfield(a1, a0 ^ z1);
	}

	__host__ __device__ static inline cuda::std::pair<uint64_t, uint64_t> to_subfield(uint64_t elm) {
		static_assert(HEIGHT > 0, "Height cannot be negative");
		uint64_t mask = BITMASK(HEIGHTTOBIT(HEIGHT - 1));
		return cuda::std::make_pair(elm & mask, (elm >> HEIGHTTOBIT(HEIGHT - 1)) & mask);
	}

	__host__ __device__ static inline uint64_t from_subfield(uint64_t a, uint64_t b) {
		static_assert(HEIGHT > 0, "Height cannot be negative");
		uint64_t res = a | (b << HEIGHTTOBIT(HEIGHT - 1));
		return res;
	}
};
...
public:
	...
	__host__ __device__ static inline uint64_t multiply(uint64_t a, uint64_t b) {
		return FanPaarTowerFieldBase<HEIGHT>::generic_multiply(a, b);
	}

	__host__ __device__ static inline uint64_t square(uint64_t a) {
		return FanPaarTowerFieldBase<HEIGHT>::generic_square(a);
	}

	__host__ __device__ static inline uint64_t inverse(uint64_t a) {
		return FanPaarTowerFieldBase<HEIGHT>::generic_inverse(a);
	}

	__host__ __device__ static inline uint64_t multiply_alpha(uint64_t a) {
...
public:
	__host__ __device__ static inline uint64_t multiply(uint64_t a, uint64_t b) { return a & b & 1; }

	__host__ __device__ static inline uint64_t square(uint64_t a) { return a & 1; }

	__host__ __device__ static inline uint64_t inverse(uint64_t a) { return a & 1; }

	__host__ __device__ static inline uint64_t multiply_alpha(uint64_t a) { return a & 1; }
};
