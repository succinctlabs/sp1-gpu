...
extern "C" void poseidon2_hash_cuda(bb31_t *in, int len, bb31_t *out,
    ...
    CUDA_OK(cudaMalloc((void **)&in_device, n * len * sizeof(bb31_t)));
    CUDA_OK(cudaMalloc((void **)&out_device, n * WIDTH * sizeof(bb31_t)));
    CUDA_OK(cudaMalloc((void **)&external_rc_device,
                       WIDTH * ROUNDS_F * sizeof(bb31_t)));
    CUDA_OK(cudaMalloc((void **)&internal_rc_device,
                       WIDTH * ROUNDS_P * sizeof(bb31_t)));

    CUDA_OK(cudaMemcpy(in_device, in, n * len * sizeof(bb31_t),
                       cudaMemcpyHostToDevice));
    CUDA_OK(cudaMemcpy(external_rc_device, external_rc,
                       WIDTH * ROUNDS_F * sizeof(bb31_t),
                       cudaMemcpyHostToDevice));
    CUDA_OK(cudaMemcpy(internal_rc_device, internal_rc,
                       WIDTH * ROUNDS_P * sizeof(bb31_t),
                       cudaMemcpyHostToDevice));
...
extern "C" void poseidon2_permute_cuda(bb31_t *in, bb31_t *out,
    ...
    CUDA_OK(cudaMemcpy(in_device, in, n * WIDTH * sizeof(bb31_t),
                       cudaMemcpyHostToDevice));
    CUDA_OK(cudaMemcpy(external_rc_device, external_rc,
                       WIDTH * ROUNDS_F * sizeof(bb31_t),
                       cudaMemcpyHostToDevice));
