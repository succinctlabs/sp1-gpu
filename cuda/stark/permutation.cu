...
template<typename F, typename EF> __global__ void PopulatePermutationRowsFlattened(
    ...
    Matrix<F> const main, EF const alpha,
    EF const beta, size_t const batch_size) {
        ...
        for (size_t i = 0 ; i < num_interactions; i += batch_size)
        {
            ...
            size_t perm_start_idx = (i / batch_size) * EF::D;
            #pragma unroll
        ...
        if (num_interactions != 0) {
            ...
            for (size_t k = 0; k < EF::D; k++) {
                ...
                permutation.values[flatten_perm_index * permutation.height + RowIdx] = cumulative_sum.value[k];
            }
        }
    }
