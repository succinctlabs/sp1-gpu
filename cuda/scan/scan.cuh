...
template<typename T> RustCudaError scan_template(T * d_out, const T * d_in, size_t n) {
    if((2 * n) <= SECTION_SIZE)
...
extern "C" RustCudaError scan_baby_bear(bb31_t * d_out, const bb31_t* d_in, size_t n) {
    return scan_template(d_out, d_in, n);
}

extern "C" RustCudaError scan_baby_bear_challenge(bb31_extension_t * d_out, 
   const bb31_extension_t  *d_in, size_t n) {
    return scan_template(d_out, d_in, n);
}