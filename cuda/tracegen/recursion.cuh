...
template<class T>
__global__ void recursion_mem_variable_generate_trace_kernel(
    MatrixViewMutDevice<T> trace,
    const sp1_recursion_core_sys::MemIo<sp1_recursion_core_sys::Block<T>>*
        events,
    uintptr_t nb_events
) {
    static const size_t COLUMNS =
        sizeof(sp1_recursion_core_sys::Block<T>) / sizeof(T);

    int i = blockIdx.x * blockDim.x + threadIdx.x;
    for (; i < nb_events; i += blockDim.x * gridDim.x) {
        sp1_recursion_core_sys::Block<T> cols;
        sp1_recursion_core_sys::mem_variable::event_to_row<T>(events[i], cols);
...
extern "C" rustCudaError_t recursion_mem_variable_generate_trace(
    MatrixViewMutDevice<bb31_t> trace,
    const sp1_recursion_core_sys::MemIo<sp1_recursion_core_sys::Block<bb31_t>>*
        events,
    uintptr_t nb_events,
    CudaStreamHandle stream_handle
) {
    ...
    static const int M = 256;
    recursion_mem_variable_generate_trace_kernel<bb31_t>
        <<<(trace.height - 1) / M + 1, M, 0, stream>>>(
            trace,
            events,
            nb_events
