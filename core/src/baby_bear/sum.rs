...
use crate::{
    cuda_runtime::{CudaSync, DeviceAllocator},
...
impl<A: DeviceAllocator> DeviceBuffer<BabyBear, A> {
...
impl<A: DeviceAllocator> ColMajorMatrixDevice<BabyBear, A> {
    pub fn column_sum<B: DeviceAllocator>(
        &self,
        results: &mut DeviceBuffer<BabyBear, B>,
    ) -> Result<(), CudaError> {
        unsafe {
            ffi::sum_baby_bear(
                self.values.as_ptr(),
