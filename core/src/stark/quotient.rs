...
use crate::device::{DeviceBuffer, RawDevicePointer};
...
pub struct DeviceQuotientValues<SC: StarkGenericConfig, P: RawDevicePointer<Data = SC::Val>> {
    pub quotient_chunks: Vec<ColMajorMatrix<P>>,
    pub quotient_chunk_domains: Vec<Dom<SC>>,
}
    ...
    A: for<'a> Air<P3EvalFolder<'a>> + MachineAir<SC::Val>,
{
    ...
    pub fn split_evals<P: RawDevicePointer<Data = SC::Val>>(
        &self,
        num_chunks: usize,
        evals: &ColMajorMatrix<P>,
    ) -> Result<Vec<ColMajorMatrix<P>>, CudaError> {
        (0..num_chunks)
    ...
    pub fn generate_quotient_values<C, P>(
        &self,
        committer: &TwoAdicFriCommitter<SC, C>,
        chips: &[&Chip<SC::Val, A>],
        pk: &StarkProvingKey<SC>,
        main_traces: &[ColMajorMatrix<P>],
        domain_and_permutation_traces: &[(Dom<SC>, ColMajorMatrix<P>)],
        permutation_challenges: &[SC::Challenge],
        folding_challenge: SC::Challenge,
        public_values: &[SC::Val],
        cumulative_sums: &[SC::Challenge],
    ) -> Result<Vec<DeviceQuotientValues<SC, P>>, CudaError>
    where
        ...
        DeviceDft<BabyBear>: Dft<P>,
    {
        ...
        for (i, chip) in chips.iter().enumerate() {
            ...
            let alloc = main_traces[i].values.allocator();
            // Compute the evaluations of the traces on the quotient domain.
            let preprocessed_on_quotient_domain = pk
                .chip_ordering
                .get(&chip.name())
                .map(|&index| pk.traces[index].to_device_in(alloc).unwrap())
                .map(|trace| {
                    committer.get_evaluations_on_domain(trace_domain, quotient_domain, &trace)
                })
                .transpose()?;
            let preprocessed_on_quotient_domain = preprocessed_on_quotient_domain
                .unwrap_or_else(|| ColMajorMatrix::<P>::empty_in(alloc));
            ...
            let quotient_flat = trace_span!("Compute quotient values").in_scope(|| unsafe {
                let mut quotient_flat = ColMajorMatrix::<P>::with_capacity_in(
                    <SC::Challenge as AbstractExtensionField<SC::Val>>::D,
                    quotient_domain.size(),
                    main_on_quotient_domain.values.allocator(),
                )
                .unwrap();
